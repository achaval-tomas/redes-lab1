...
PREFIX = "https://"
HTTP_PORT = 443   # El puerto por convencion para HTTP,
...
def connect_to_server(server_name):
    ...
    # Buscar direccion ip
    address_info = socket.getaddrinfo(server_name, HTTP_PORT)

    if len(address_info) == 0:
        raise Exception("Error al obtener dirección ip del servidor")
    ...
    # Envolvemos el socket con ssl, sin utilizar ningún archivo de
    # certificaciones, lo cual implica que aceptamos cualquier certificado
    # (lo que en la práctica es inseguro).
...
def send_request(connection, server_name):
    """
    ...
    # Modificamos HTTP_REQUEST para que sea válido (el nombre
    # del host va en un header, no después del GET).
    # Codificamos el nombre con IDNA para que sea ascii válido,
    # más información en
    # https://en.wikipedia.org/wiki/Internationalized_domain_name
    request = HTTP_REQUEST % server_name.encode("idna")
...
def download(url, filename):
    ...
    try:
        ...
        send_request(connection, server)
