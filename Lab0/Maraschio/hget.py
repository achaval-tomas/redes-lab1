...
def send_request(connection, url):
    ...
    # quote special characters using the %xx escape.
    # do not quote '/' and ':' to preserve the "http://" part
    encoded_url = urllib.parse.quote(url, safe="/:").encode()

    connection.send(HTTP_REQUEST % encoded_url)
